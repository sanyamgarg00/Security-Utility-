...
from phonenumbers import parse, carrier, geocoder, timezone
import phonenumbers
...
class PhoneTracker:
    def __init__(self, parent):
        ...
        self.label = ttk.Label(self.frame, text="Phone Number Tracker", font=("Arial", 22, "bold"))
        self.label.grid(row=0, column=0, columnspan=4, pady=20, sticky='n')

        self.phone_number_label = ttk.Label(self.frame, text="Enter Phone Number:", font=("Arial", 16, "bold"))
        ...
        self.track_button = ttk.Button(self.frame, text="Track Number", command=self.track_phone_number)
        self.track_button.grid(row=1, column=2, pady=10)

        self.results_text = ScrolledText(self.frame, width=40, height=10)
        self.results_text.grid(row=2, column=0, columnspan=3, pady=20)
    ...
    def track_phone_number(self):
        ...
        enter_number = self.phone_number_entry.get()
        try:
            ...
            self.results_text.insert(tk.END, f"Country: {locate}\n")
            self.results_text.insert(tk.END, f"Operator: {operator}\n")
            self.results_text.insert(tk.END, f"Timezone: {time}\n")

            geolocator = Nominatim(user_agent="geoapiExercises")
            location = geolocator.geocode(locate)
            if location:
                lng, lat = location.longitude, location.latitude
                self.results_text.insert(tk.END, f"Longitude: {lng}\n")
                self.results_text.insert(tk.END, f"Latitude: {lat}\n")

                obj = TimezoneFinder()
                result = obj.timezone_at(lng=lng, lat=lat)
                home = timezone(result)
                local_time = datetime.now(home)
                current_time = local_time.strftime("%I:%M %p")
                self.results_text.insert(tk.END, f"Phone Time: {current_time}\n")
            else:
                self.results_text.insert(tk.END, "Location information not available for this number.\n")

        except phonenumbers.NumberParseException:
            self.results_text.insert(tk.END, "Invalid phone number format.\n")
...
def main():
    ...
    phone_tracker_tab = ttk.Frame(tab_control)
    ...
    tab_control.add(phone_tracker_tab, text="Phone Tracker")
    ...
    PhoneTracker(phone_tracker_tab)
